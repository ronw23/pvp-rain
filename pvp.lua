...
local production_score = require("production-score-rain")
...
local set_team = function(player_index, team)
  ...
  if team then
    team.members[player_index] = player
...
local gui_functions =
{
  ...
  admin_button = function(event, param)
    ...
    frame.visible = true
    register_gui_action(frame.add{type = "button", caption = {"end-round"}, tooltip = {"end-round-tooltip"}}, {type = "admin_end_round"})
    register_gui_action(frame.add{type = "button", caption = {"reroll-round"}, tooltip = {"reroll-round-tooltip"}}, {type = "admin_reroll_round"})
    register_gui_action(frame.add{type = "button", caption = {"restart-round"}, tooltip = {"restart-round-tooltip"}}, {type = "admin_restart_round"})
    register_gui_action(frame.add{type = "button", caption = {"admin-change-team"}, tooltip = {"admin-change-team-tooltip"}}, {type = "spectator_join_team_button"})
  end,
  ...
  admin_restart_round = function(event, param)
    ...
    end_round()
    start_round(script_data.round_seed)
    return
  end,
...
function create_config_gui(player)
  ...
  local holding_table = upper_frame.add{type = "frame", direction = "horizontal", style = "window_content_frame"}
  local team_lobby = holding_table.add{type = "frame", direction = "vertical", style = "bordered_frame"}
  team_lobby.style.vertically_stretchable = true
  team_lobby.add{type = "label", caption = {"teams"}, style = "caption_label"}
  if admin then
    local button = team_lobby.add{type = "button", caption = {"add-team"}, tooltip = {"add-team-tooltip"}, enabled = #script_data.config.teams < 24, style = "dialog_button"}
  ...
  local team_settings = holding_table.add{type = "frame", direction = "vertical", style = "bordered_frame"}
  ...
  local game_settings = holding_table.add{type = "frame", direction = "vertical", style = "bordered_frame"}
  ...
  add_new_config_gui(script_data.config.game_config, game_settings, admin)
  ...
  local victory = other_flow.add{type = "frame", direction = "vertical", style = "bordered_frame"}
  victory.add{type = "label", caption = {"victory-conditions"}, style = "caption_label"}
  victory.style.vertically_stretchable = true
  add_victory_gui(script_data.config.victory, victory, admin)
  local disable_items = other_flow.add{type = "frame", style = "bordered_frame", direction = "vertical"}
  disable_items.add{type = "label", caption = {"disabled-items"}, style = "caption_label"}
...
function prepare_next_round()
  for k, config in pairs (script_data.elements.config) do
    if config and config.valid then
      config.destroy()
    end
  end
  script_data.elements.config = {}
  script_data.setup_finished = false
...
function set_player(player, team, mute)
  ...
  local position = surface.find_non_colliding_position("player", force.get_spawn_position(surface), 0, 2)
...
function choose_joining_gui(player)
  ...
  local title_label = title_flow.add{type = "label", caption = {"pick-join"}, style = "heading_1_label"}
  local title_pusher = title_flow.add{type = "flow"}
  ...
  if player.force.name ~= "player" then
    local title_close_button = title_flow.add{type = "sprite-button", style = "close_button", sprite = "utility/close_white"}
...
function toggle_balance_options_gui(player)
  ...
  local inner = frame.add{type = "frame", style = "window_content_frame"}
  local scrollpane = inner.add{type = "scroll-pane"}
  ...
  for modifier_name, array in pairs (modifier_list) do
    local flow = big_table.add{type = "frame", style = "bordered_frame", direction = "vertical"}
...
function create_disable_frame(gui)
  local disable_table = gui.add{type = "table", column_count = 5}
...
function start_round(seed)
  script_data.round_seed = seed or get_random_seed()
  script_data.random = game.create_random_generator(script_data.round_seed)
  script_data.ready_start_tick = nil
  destroy_config_for_all()
  prepare_next_round()
end
...
function update_oil_harvest_frame(player)
  ...
  if script_data.config.game_config.time_limit > 0 then
    frame.add{type = "label", caption = {"time_left", get_time_left()}}
  end
...
function update_kill_score_frame(player)
  ...
  if script_data.config.victory.required_kill_score > 0 then
    frame.add{type = "label", caption = {"", {"required_kill_score"}, {"colon"}, " ", util.format_number(script_data.config.victory.required_kill_score)}}
  end
...
function update_space_race_frame(player)
  ...
  if script_data.config.victory.required_rockets_sent > 0 then
    frame.add{type = "label", caption = {"", {"required_rockets_sent"}, {"colon"}, " ", util.format_number(script_data.config.victory.required_rockets_sent)}}
  end
...
function check_update_kill_score()
  if script_data.team_won then return end
  if not script_data.config.victory.kill_score then return end
  local scores = get_kill_scores()
  ...
  local required = script_data.config.victory.required_kill_score
  if required > 0 then
...
function check_update_space_race_score()
  ...
  local required = script_data.config.victory.required_rockets_sent
  if required > 0 then
    for team_name, score in pairs (scores) do
      if score >= required then
        team_won(team_name)
...
function update_timers()
  local caption = {"time_left", get_time_left()}
  for k, timer in pairs (script_data.timers) do
    if timer.valid then
      timer.caption = caption
    end
  end
end
...
pvp.on_configuration_changed = function(data)
  ...
  script_data.round_seed = script_data.round_seed or game.surfaces[1].map_gen_settings.seed
  script_data.random = script_data.random or game.create_random_generator(script_data.round_seed)
end
...
local script_events =
{
  ...
  [defines.events.on_player_display_scale_changed] = on_player_display_resolution_changed,
